...
sys.path.append(os.path.abspath(os.path.join(currentUrl, 'deep_sort')))
...
class DeepsortTracker():
    def __init__(self):
        rospy.loginfo("Initialize YOLO_V5 - Deepsort Tracker")

        # ROS parameters
        weights_name = rospy.get_param('~weights_name', 'yolov5/weights/yolov5s.pt')
        config_name = rospy.get_param('~config_path', 'configs/deep_sort.yaml')
        ckpt_name = rospy.get_param('~ckpt_path', 'deep_sort/deep/checkpoint/ckpt.t7')
        self.image_topic = rospy.get_param('~image_topic', '/image_raw')
        self.tracked_objects_topic = rospy.get_param('~tracked_objects_topic')
        self.published_image_topic = rospy.get_param('~tracked_image_topic')
        ...
        # Define publishers
        self.pub_ = rospy.Publisher(self.tracked_objects_topic, BoundingBoxes, queue_size=10)
    ...
    def image_callback(self, msg):
        ...
        with torch.no_grad():
            detections = self.detector(input_img, augment=False)[0]
            detections = non_max_suppression(detections, self.confidence_th, self.iou_thres, classes=self.class_name, agnostic=False)

        det = detections[0]  # for video, bz is 1
        if det is not None and len(det):  # det: (#obj, 6)  x1 y1 x2 y2 conf cls
            # rescale to original image
            det[:, :4] = scale_coords(input_img.shape[2:], det[:, :4], self.cv_image.shape).round()

            bbox_xywh = xyxy2xywh(det[:, :4]).cpu()
            confs = det[:, 4:5].cpu()
            clss = det[:, 5:6].cpu()
            outputs = self.deepsort.update(bbox_xywh, confs, self.cv_image)
